...
export const useCashdeskSession = () => {
  const [currentSession, setCurrentSession] = useState<CashdeskSession | null>(null);
  ...
  const startSession = async (openingFloat: number) => {
    ...
    if (sessionStats) {
      ...
      localStorage.setItem(`cashdesk_stats_${user.id}`, JSON.stringify(updatedStats));
    }
  };
  ...
  const endSession = async (session: CashdeskSession) => {
    ...
    if (sessionStats) {
      const updatedStats = {
        ...
        totalTransactions: sessionStats.totalTransactions + session.transactionCount,
        averageTransactionValue: sessionStats.totalTransactions > 0 
          ? (sessionStats.totalSales + session.totalSales) / (sessionStats.totalTransactions + session.transactionCount)
          : 0,
    ...
    toast({
      title: "Session Ended",
      description: `Session completed with ₦${session.totalSales.toFixed(2)} in sales`,
    });
  };
  ...
  const addPettyCashTransaction = (transaction: Omit<PettyCashTransaction, 'id' | 'timestamp' | 'cashierId'>) => {
    if (!currentSession || !user) return;
    ...
    const updatedTransactions = [...currentSession.pettyCashTransactions, pettyCashTransaction];
